...
export const coverageZone = pgTable(
  "coverage_zone",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id")
    ...
    type: text("type", { enum: ["radius", "polygon"] }).notNull().default("radius"),
  ...
  (t) => [index("coverage_zone_org_idx").on(t.organizationId)]
);
...
export const todo = pgTable(
  ...
  (t) => [index("todo_org_idx").on(t.organizationId)]
);

export const cajaNap = pgTable(
  "cajas_nap",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id")
      .notNull()
      .references(() => organization.id, { onDelete: "cascade" }),
    napCode: text("nap_code").notNull(),
    puertos: text("puertos").notNull(),
    ubicacionRaw: text("ubicacion_raw"),
    latitud: doublePrecision("latitud").notNull(),
    longitud: doublePrecision("longitud").notNull(),
    red: text("red").notNull(),
    estado: text("estado", { enum: ["ACTIVO", "INACTIVO", "SATURADO"] })
      .notNull()
      .default("ACTIVO"),
    notes: text("notes"),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  ...
  (t) => [
    index("caja_nap_org_idx").on(t.organizationId),
    index("caja_nap_org_red_idx").on(t.organizationId, t.red),
    index("caja_nap_org_estado_idx").on(t.organizationId, t.estado),
  ]
);

export const registroCoberturaCliente = pgTable(
  "registro_cobertura_clientes",
  {
    id: text("id").primaryKey(),
    organizationId: text("organization_id")
      .notNull()
      .references(() => organization.id, { onDelete: "cascade" }),
    clientifyLeadId: text("clientify_lead_id"),
    clienteNombre: text("cliente_nombre"),
    clienteLatitud: doublePrecision("cliente_latitud").notNull(),
    clienteLongitud: doublePrecision("cliente_longitud").notNull(),
    napAsignadaId: text("nap_asignada_id").references(() => cajaNap.id, {
      onDelete: "set null",
    }),
    distanciaLinealM: doublePrecision("distancia_lineal_m").notNull(),
    distanciaRutaM: doublePrecision("distancia_ruta_m"),
    estadoCobertura: text("estado_cobertura", {
      enum: ["COBERTURA_FACTIBLE", "COBERTURA_DISTANTE_EVALUACION"],
    }).notNull(),
    fechaConsulta: timestamp("fecha_consulta").notNull().defaultNow(),
  },
  (t) => [index("reg_cob_org_idx").on(t.organizationId, t.fechaConsulta)]
);

/* ============================================================
 * Campañas y Recordatorios de WhatsApp (003)
 * ============================================================ */
...
/* ============================================================
 * Módulo de Agenda, Visitas Técnicas y Citas (004-appointments-calendar)
 * ============================================================ */
...
export const appointment = pgTable(
  ...
  (t) => [
    ...
    index("appointment_org_assigned_idx").on(
      t.organizationId,
      t.assignedUserId
    ),
...
/* ============================================================
 * Módulo de Etiquetas y Respuestas Rápidas (005-tags-and-quick-replies)
 * ============================================================ */
...
export const quickReply = pgTable(
  "quick_reply",
  {
    ...
    message: text("message").notNull(), // Contenido completo
    createdAt: timestamp("created_at").notNull().defaultNow(),
    updatedAt: timestamp("updated_at").notNull().defaultNow(),
  },
  (t) => [
    uniqueIndex("quick_reply_org_shortcut_uq").on(
      t.organizationId,
      t.shortcut
    ),
