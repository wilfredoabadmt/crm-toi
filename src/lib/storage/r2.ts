...
/**
 * Descarga el contenido de un objeto desde Cloudflare R2.
 */
export async function getR2Object(key: string): Promise<{ body: Uint8Array; contentType?: string }> {
  ...
  if (!res.Body) {
    throw new Error("Objeto no encontrado o vacío en R2");
  }
...
/**
 * Elimina un objeto de Cloudflare R2 a partir de su llave o URL.
 */
export async function deleteFromR2(keyOrUrl: string): Promise<void> {
  try {
    ...
    let key = keyOrUrl;
    const match = keyOrUrl.match(/agent-media\/.+$/);
    if (match) {
      key = match[0];
    }
    ...
    console.warn("[R2] Error al eliminar archivo de R2 (ignorado si no existe):", err);
  }
}
