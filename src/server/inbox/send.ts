...
/**
 * Envía un mensaje de imagen por WhatsApp utilizando una URL pública (Cloudflare R2).
 */
export async function sendImage(input: {
  ...
  if (!isWindowOpen(row.conversation.lastInboundAt)) {
    throw new SendError(
      "window_closed",
      "La ventana de 24 horas está cerrada; no se pueden enviar imágenes libres sin plantilla"
    );
  }
  ...
  let publicImageUrl = input.imageUrl;
  const matchKey = input.imageUrl.match(/agent-media\/.+$/);
  if (matchKey) {
    const appBaseUrl = (process.env.APP_BASE_URL ?? "https://crmtoi.clientify.click").replace(/\/+$/, "");
    publicImageUrl = `${appBaseUrl}/api/agent/media/file?key=${encodeURIComponent(matchKey[0])}`;
  }

  const imagePayload: { link: string; caption?: string } = { link: publicImageUrl };
  if (input.caption?.trim()) {
    imagePayload.caption = input.caption.trim();
  }
