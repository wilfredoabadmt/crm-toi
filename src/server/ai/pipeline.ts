...
export async function runAgentTurn(conversationId: string): Promise<void> {
  ...
  let locationContext = "";
  if (lastInbound.text) {
    const { parseCoordinatesOrLink, verifyClientCoverageNap } = await import("@/server/coverage/naps");
    const parsedCoords = parseCoordinatesOrLink(lastInbound.text);

    if (parsedCoords) {
      const { lat, lng } = parsedCoords;
      const cov = await verifyClientCoverageNap(organizationId, {
        latitud: lat,
        longitud: lng,
      });

      if (cov.factible) {
        // Cobertura factible: pedir requisitos y derivar a humano
        const napInfo = cov.nap_asignada
          ? `Caja NAP "${cov.nap_asignada.codigo}" (Red: ${cov.nap_asignada.red})`
          : "Nodo de distribución principal";
        locationContext = `

[SISTEMA - VERIFICACIÓN DE COBERTURA NAP EN TIEMPO REAL]
El cliente compartió su ubicación GPS (Latitud: ${lat}, Longitud: ${lng}).
RESULTADO: ¡COBERTURA FACTIBLE CONFIRMADA!
- Punto de conexión: ${napInfo}
- Distancia exacta por recorrido de calles: ${cov.distancia_ruta_metros} metros (Distancia lineal: ${cov.distancia_lineal_metros}m)
- Nota del sistema: ${cov.mensaje_para_agente_ia}
INSTRUCCIÓN DE RESPUESTA:
1. Celebra y confirma con entusiasmo, amabilidad y tono muy profesional que SÍ TIENE COBERTURA EXCELENTE de fibra óptica en su domicilio.
2. Menciónale que se encuentra a solo ${cov.distancia_ruta_metros} metros de nuestro nodo activo (${cov.nap_asignada?.codigo ?? "nodo cercano"}).
3. Indícale que para agendar la instalación requerimos únicamente: (a) Fotocopia o foto de C.I. y (b) Factura de servicio básico (luz o agua).
4. Cierra invitándolo persuasivamente a agendar su turno de instalación para esta semana.`;
      } else {
        // Cobertura no factible: enviar mensaje específico sin mostrar datos técnicos
        locationContext = `

[SISTEMA - VERIFICACIÓN DE COBERTURA NAP EN TIEMPO REAL]
El cliente compartió su ubicación GPS (Latitud: ${lat}, Longitud: ${lng}).
RESULTADO: COBERTURA DISTANTE (EN EVALUACIÓN TÉCNICA).
- Punto activo más cercano: ${
          cov.nap_asignada?.codigo ?? "N/A"
        } (Distancia por calles: ${cov.distancia_ruta_metros} metros, supera el estándar directo de 300m)
- Nota del sistema: ${cov.mensaje_para_agente_ia}
INSTRUCCIÓN DE RESPUESTA:
Gracias por compartir tu ubicación. Por el momento no contamos con cobertura en esa zona. Si deseas, podemos registrar tu solicitud para futuras ampliaciones de cobertura.`;
      }
    }
  } else {
    // No es una ubicación: continuar con el flujo normal
    locationContext = "";
  }
