...
/** PUT: Actualiza el nombre, categoría, regla o reemplaza el archivo en Cloudflare R2. */
export const PUT = withAuth(async (session, req: Request) => {
  try {
    ...
    const name = (formData.get("name") as string | null)?.trim();
    const rule = (formData.get("rule") as string | null)?.trim();
    const category = (formData.get("category") as AgentMediaCategory | null) ?? undefined;
    const file = formData.get("file") as File | null;
    ...
    if (rule) updatePayload.rule = rule;

    // Si se adjunta un nuevo archivo, subirlo a R2 y eliminar el archivo antiguo
    if (file && file.size > 0) {
      const arrayBuffer = await file.arrayBuffer();
      const buffer = Buffer.from(arrayBuffer);

      const newPublicUrl = await uploadToR2({
        file: buffer,
        filename: file.name || "imagen.jpg",
        mimeType: file.type || "image/jpeg",
      });

      updatePayload.url = newPublicUrl;
      updatePayload.filename = file.name || "imagen.jpg";
      updatePayload.mimeType = file.type || "image/jpeg";

      // Obtener URL previa para limpiar en R2
      const existingList = await getAgentMediaByOrg(session.organizationId);
      const oldItem = existingList.find((i) => i.id === id);
      if (oldItem?.url) {
        void deleteFromR2(oldItem.url);
      }
    }

    const updatedItem = await updateAgentMedia(session.organizationId, id, updatePayload);
    if (!updatedItem) {
      return apiError(404, "not_found", "Recurso no encontrado o sin permisos.");
    }

    return NextResponse.json({ ok: true, item: updatedItem });
  } catch (err) {
    console.error("[api/agent/media] Error al actualizar recurso:", err);
    return apiError(
      500,
      "server_error",
      err instanceof Error ? err.message : "Error al actualizar el recurso de imagen."
    );
  }
});

/** DELETE: Elimina una imagen del catálogo del agente y de Cloudflare R2. */
