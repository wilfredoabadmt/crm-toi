...
export function AppShell({ user, children }: { user: User; children: ReactNode }) {
  ...
  const SidebarItem = ({ href, icon: Icon, label }: { href: string; icon: any; label: string }) => {
    ...
    const content = (
      <Link
        ...
        className={cn(
          'sidebar-nav-item',
          isExpanded ? 'w-full justify-start' : 'w-9 justify-center',
  ...
  return (
    <div className="flex h-full">
      {/* Sidebar estilo glassmorphism oscuro como landing */}
      <aside 
        className={cn(
          "flex shrink-0 flex-col py-3 transition-all duration-300",
          "bg-sidebar-bg border-r border-white/8",
      ...
      >
        <div className={cn("mb-6 flex items-center", isExpanded ? "px-2 justify-between" : "justify-center")}>
          <Link href="/" className="flex items-center gap-2 rounded-lg overflow-hidden hover:opacity-80 transition-opacity w-full">
            <img 
              src={brand.data?.logo ?? '/logo.png'} 
              alt="Logo" 
              className="h-8 w-8 object-contain shrink-0 logo-glow" 
            />
            {isExpanded && (
              <span className="font-extrabold text-sm text-sidebar-text-active tracking-tight whitespace-normal break-words leading-tight flex-1">
                {brand.data?.name ?? 'TOI'}
              </span>
            )}
          </Link>
          ...
          <Dropdown>
            <DropdownTrigger asChild>
              <button 
                ...
                className={cn(
                  "rounded-xl ring-offset-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring flex items-center transition-all hover:bg-white/5",
              ...
              >
                ...
                {isExpanded && (
                  <div className="flex flex-1 flex-col min-w-0">
                    <span className="text-xs font-semibold text-sidebar-text-active truncate">{user.name}</span>
                    <span className="text-[10px] text-sidebar-text truncate">{user.email}</span>
                  </div>
                )}
              </button>
            </DropdownTrigger>
            <DropdownContent side={isExpanded ? "top" : "right"} align={isExpanded ? "start" : "end"} className="w-56 glass-card !rounded-xl">
              <DropdownLabel>
                {user.name}
